---
layout: default
title: CV
permalink: /cv/
description: Education, teaching, research, and industry experience.
---

## Education

* M.S. in Mathematics, Boğaziçi University
* B.S. in Electrical and Electronics Engineering, Boğaziçi University
* B.S. in Physics, Boğaziçi University

## Teaching and academic work

* Teaching Assistant, Boğaziçi University
  * MATH 102 - Multivariable Calculus
  * MATH 105 - Introduction to Finite Mathematics
  * MATH 412 - Axiomatic Set Theory
* Master's student in mathematics under the supervision of [Assoc. Prof. Ayhan Günaydın](https://web.bogazici.edu.tr/ayhan.gunaydin)

## Research experience

* Undergraduate Research Assistant, ETH Zurich
  * Supervisor: [Prof. Ender Konukoglu](https://people.ee.ethz.ch/~kender)
  * Worked on the spatial inductive bias of convolutional neural networks and the unsupervised image restoration setting often called Deep Image Prior.
* Undergraduate Research Assistant, Boğaziçi University
  * Supervisor: [Assoc. Prof. Ahmet Öncü](https://ee.bogazici.edu.tr/ahmet-oncu)
  * Worked on digital circuit design and verification, including graphical tools for programming digital circuits.

## Industry and outreach

* Cryptanalyst Intern, [BİLGEM](https://bilgem.tubitak.gov.tr/en)
  * Studied and implemented linear and differential cryptanalysis for DES-like block ciphers.
  * Prepared reports on Grover's and Shor's algorithms and their implications for RSA.
  * Prepared reports on lattice-based cryptography and the Fiat-Shamir transform.
* R&D Intern, [SESTEK](https://www.sestek.com)
  * Studied voice activity detection with recurrent neural networks and handcrafted features, and designed benchmark experiments for system comparison.
* Physics Instructor, [TÜBİTAK](https://www.tubitak.gov.tr/en)
  * Taught and ran problem sessions in electromagnetism, mechanics, and modern physics for students preparing for the Turkish physics olympiad program.
